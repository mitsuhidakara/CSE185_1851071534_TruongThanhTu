...
if(isset($_POST['add_achievement'])){
	if($mysqli->query("insert achievements values(
		'',
		'".$_SESSION['profile_session']."',
		'".base64_encode($_POST['achievements'])."'
	)")){
		echo "<script>alert('Adding success!')</script>";
		echo "<script>window.open('index.php?page=profile','_self')</script>";
	}else{
...
if(!empty($_GET['achievements_delete_id'])){
	if($mysqli->query("delete from achievements where id = '".$_GET['achievements_delete_id']."'")){
		echo "<script>alert('Delete success!')</script>";
		echo "<script>window.open('index.php?page=profile','_self')</script>";
	}else{
		$wer = '<p style="color:#f00;">Something wrong! please try again.</p>';
...
<?php if(!empty($wer)){ echo $wer; }?>
	<div class="row">
	  ...
	  <div class="col-sm-4">
		  ...
		  <form action="" method="post">
			  <p>Achievements information</p>

			  <div class="form-group">
				<label>Achievements:</label>
				<input type="text" class="form-control" placeholder="Achievements" name="achievements" required>
			  </div>			
			  <button name="add_achievement" type="submit" class="btn btn-default">Add Achievement</button>
		  </form>
		  <table class="table table-bordered" style="margin-top:20px;">
		  <?php
			  $query = $mysqli->query("select * from achievements where a_id = '".$_SESSION['profile_session']."'");
			  while($row = mysqli_fetch_assoc($query)){
		  ?>
				<tr>
				  <td>
				  ...
				  <td>
					  <a href="index.php?page=profile&achievements_delete_id=<?php echo $row['id']; ?>">delete</a>
				  </td>
				</tr>
		  <?php } ?>	  
		  </table>
